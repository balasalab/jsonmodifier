...
class ComposeJson(ReadJson):
	...
	def load_output_template(self):
		if self.check_data_type(self.output_template) == "dict":
			...
			res=self.read_list_data(self.output_template)
			return (res)
	...
	def read_list_data(self, key, composed_key=None):
		logger.debug("read list data "+ str(key))
		temp=[]
		if isinstance(key[0], dict):
			res = self.read_dict_data(key[0], [], composed_key)
			temp.append(res)
		else:
			logger.debug("Key type list")
			...
			for k in keys:
				if self.isSubSet(composed_key, k):
					...
					logger.debug("composed key value is not matched")
		return temp
